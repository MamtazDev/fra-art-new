            ...
            <div className="card nft-items nft-primary rounded-md shadow overflow-hidden mb-1 p-3">
                <div className="d-flex justify-content-between">
                    ...
                    <span className="like-icon shadow-sm">
                        {likeStatus ? <div onClick={() => actionLike()} className="text-muted icon-active">
                            <i className="mdi mdi-18px mdi-heart mb-0"></i>
                        </div> : <div onClick={() => actionLike()} className="text-muted icon">
                            <i className="mdi mdi-18px mdi-heart mb-0"></i>
                        </div>}
                    </span>
                </div>
                ...
                <div className="card-body content position-relative p-0 mt-3">
                    ...
                    <div className="d-flex justify-content-between mt-2">
                        {
                            item.pair
                                ? (<small className="rate fw-bold"><span>Price: </span><span>{item.pair.price} {process.env.REACT_APP_COIN}</span></small>)
                                : (item.auction
                                    ? (<small className="rate fw-bold"><span>Highest Bid: </span><span>{item.auction.price} {process.env.REACT_APP_COIN}</span></small>)
